...
- **Student numbers** (e.g. `2025-00001-MN-0`) are generated by the system upon the first approved enrollment — not during registration.
- ITech diploma programs and OUS are **not included** in this version.
- **RLS is disabled** on Supabase — all authorization is handled by Laravel's `RoleMiddleware`.
