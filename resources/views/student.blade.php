...
<head>
    ...
    <title>Student Dashboard | PUP Enrollment Portal</title>
            ...
            @forelse ($applications as $app)
                <div class="card mb-4 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        {{-- Approved: show section and subjects --}}
                                            ...
                                            @foreach ($app->subjectEnrollments as $enrollment)
                                                <tr>
                                                    <td class="fw-semibold">
                                                        {{ $enrollment->subjectOffering->subject->code ?? 'N/A' }}
                                                    </td>
                                                    <td>{{ $enrollment->subjectOffering->subject->title ?? 'N/A' }}</td>
                                                    <td class="text-center">
                                                        {{ $enrollment->subjectOffering->subject->units ?? '-' }}
                                                    </td>
                                                    <td>{{ $enrollment->subjectOffering->schedule ?? 'TBA' }}</td>
                                                    <td>{{ $enrollment->subjectOffering->room ?? 'TBA' }}</td>
                                                    <td>
                                                        {{ $enrollment->subjectOffering->faculty->full_name ?? 'TBA' }}
                                                    </td>
                                                    <td>
                                                        <span class="badge rounded-pill"
                                                            style="background-color: {{ $enrollment->status->color ?? '#6c757d' }}">
